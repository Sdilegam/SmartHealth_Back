...
public class DoctorRepository(SmartHealthContext context) : BaseRepository<Doctor>(context), IDoctorRepository
{
    ...
    public List<WorkingHoursDTO> GetAvailabilityList(int id, AvailabilityRequestRange range)
    {
        ...
        for (int i = 1; i <= 7; i++)
        {
            ...
            availabilityQuery = availabilityQuery.Where(a => a.Days.HasFlag((DaysOfWeekEnum)i));
            availabilityList.AddRange(availabilityQuery.Select(a => new WorkingHoursDTO
                {
                    DaysOfWeek = [i], EndTime = a.EndTime.ToString("HH:mm"), StartTime = a.StartTime.ToString("HH:mm")
                })
                .ToList());
    ...
    private List<SlotsTaken> SlotsTakenPerDay(List<Appointment> list)
    {
        ...
        foreach (Appointment item in list)
        {
            ...
            {
                slotList.Add(new SlotsTaken { EndDate = end.AddMinutes(5), StartDate = start.AddMinutes(-5) });
                added = true;
            }
        }
        if (!added) slotList.Add(new SlotsTaken { EndDate = end.AddMinutes(5), StartDate = start.AddMinutes(-5) });
        return slotList;
    }
}